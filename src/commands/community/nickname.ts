import { ChatInputCommandInteraction, SlashCommandBuilder, EmbedBuilder, GuildMember, PermissionFlagsBits, User } from 'discord.js';
...
async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
    try {
        ...
        if (newNickname && !isValidNickname(newNickname)) {
            return await sendErrorResponse(interaction, "Invalid nickname. Only letters, numbers, spaces, underscores, and hyphens are allowed.");
        }
        ...
        if (targetUser && targetUser.id !== member.id) {
            ...
            if (!hasModeratorRole && !member.permissions.has(PermissionFlagsBits.ManageNicknames) && !isServerOwner) {
                return await sendErrorResponse(interaction, "You don't have permission to manage other users' nicknames. You need a moderator role or the 'Manage Nicknames' permission.");
            }

            const botMember = interaction.guild.members.me;
            if (!botMember?.permissions.has(PermissionFlagsBits.ManageNicknames)) {
                return await sendErrorResponse(interaction, "I don't have permission to manage nicknames. Please ask an administrator to grant me the 'Manage Nicknames' permission.");
            }
            ...
            if (!targetMember.manageable) {
                return await sendErrorResponse(interaction, `I cannot change ${targetUser.tag}'s nickname due to role hierarchy. Their highest role is equal to or higher than mine.`);
            }

            if (member.roles.highest.position <= targetMember.roles.highest.position && member.id !== interaction.guild.ownerId) {
                return await sendErrorResponse(interaction, "You cannot change the nickname of someone with an equal or higher role.");
            }
            ...
            if (member.id === interaction.guild.ownerId && targetMember.id === member.id) {
                const result = await changeNickname(targetMember, newNickname, undefined, undefined, false, true);
                const embed = createNicknameEmbed(result, interaction);
...
async function changeNickname(
    ...
    try {
        const auditLogReason = isModAction && moderator ? `Changed by ${moderator}: ${reason}` : undefined;
        ...
        if (error.code === 50013) {
            if (isServerOwner) {
                errorReason = "Server owners cannot change their own nicknames through bots. You'll need to change it manually through Discord's interface.";
            } else {
...
function createNicknameEmbed(result: NicknameResult, interaction: ChatInputCommandInteraction): EmbedBuilder {
        ...
        if (result.isModAction) {
            if (result.newNickname) {
                embed
                    ...
                    .setDescription(`**${result.targetUser}**'s nickname has been changed to **${result.newNickname}**`)
                    .setColor(0x00ff00);
            } else {
                embed
...
async function sendToLoggingChannel(
    ...
    try {
        ...
        if (logChannel && logChannel.isTextBased()) {
            ...
            console.log(`🔧 Nickname changed by ${result.moderator}: ${result.targetUser} -> "${result.newNickname || 'cleared'}" (Reason: ${result.reason})`);
        }
    } catch (error) {
